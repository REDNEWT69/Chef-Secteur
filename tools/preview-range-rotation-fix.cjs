...
replaceOnce('selection par crédits et rotation LRU',`function selectionNeed(pool,usable,target,max){
...
function chooseStores(pool,usedKeys,lastUsedWeek,targetCount,creditBudget){
  ...
  /* Tant qu'un magasin n'a jamais été réellement placé dans la période, il reste frais.
     Le score ne départage que des magasins du même niveau de fraîcheur. */
  const fresh=pool.filter(s=>!keys.has(storeKey(s))&&!usedKeys.has(storeKey(s))).sort((a,b)=>scoreOf(b)-scoreOf(a));
  for(const s of fresh)add(s);
  /* Quand tous les magasins frais qui tiennent dans le budget ont été considérés,
     reprendre le moins récemment utilisé. Aucune remise à zéro brutale du cycle. */
...
`,`  /* Conserver ici l'ordre de sélection (forcé → frais → LRU). L'optimisation
     géographique se fait ensuite dans chaque journée ; la faire avant le placement
     pouvait remettre un magasin déjà vu devant un magasin encore frais. */
`);
...
`    const limits=selectionNeed(pool,usable,Number(state.settings.target)||20,max);
    const chosen=chooseStores(pool,new Set(),new Map(),limits.targetCount,limits.capacityCredits),built=buildWeekUnique(chosen,usable);ensureForcedPlaced(built);const visits=countPlan(built.plan,usable);`);
...
`      const limits=selectionNeed(pool,usable,target,max);
      const chosen=chooseStores(pool,usedKeys,lastUsedWeek,limits.targetCount,limits.capacityCredits),built=buildWeekUnique(chosen,usable);ensureForcedPlaced(built);const plan=built.plan,weekSeen=new Set();`);

replaceOnce('version rotation',`rotation:'hard-unique-v7'`,`rotation:'lru-credit-v8'`);

fs.writeFileSync(file,source);
console.log('PREVIEW PATCH APPLIED: rotation LRU persistante + sélection hebdomadaire bornée en crédits.');
