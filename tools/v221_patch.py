...
for path in ROOT.rglob('*'):
    if not path.is_file() or '.git' in path.parts:
        continue
    if path.suffix.lower() not in {'.js','.cjs','.html','.json','.yml','.yaml','.md'}:
        continue
    try:
...
# Contrôles de contrat avant commit.
if OLD in ''.join(p.read_text(encoding='utf-8',errors='ignore') for p in ROOT.rglob('*') if p.is_file() and '.git' not in p.parts):
    raise SystemExit('ancienne révision encore présente')
