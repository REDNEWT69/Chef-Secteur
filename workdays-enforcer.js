(function(){
  ...
  function sanitizePlan(){
    try{
      ...
      const keep=selected();
      for(const d of DAYS)if(!keep.has(d))state.plan[d]=[];
    }catch(e){}
  }
  ...
  function sanitizeArchive(){
    try{
      ...
      for(const k of Object.keys(a)){
        ...
        for(const d of DAYS){
          if(!keep.has(d)&&a[k].plan[d]&&a[k].plan[d].length){
            a[k].plan[d]=[];
            changed=true;
          }
        }
  ...
  function filterTabs(){
    try{
      ...
      document.querySelectorAll('#dayTabs .periodDayTab').forEach(function(b){
        ...
        b.style.display=keep.has(name)?'':'none';
      });
    }catch(e){}
  }
  ...
  document.addEventListener('change',function(e){
    if(e.target&&e.target.matches&&e.target.matches('[data-day]')){
      setTimeout(function(){sanitizePlan();sanitizeArchive();filterTabs()},20);
    }
  });
