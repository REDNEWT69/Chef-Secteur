...
function emptyShareLabel(merged,family){return merged?'Aucune photo pour cette visite.':'Aucune photo '+String(family||'').toUpperCase()+' pour cette visite.'}
function shareButtonLabel(total,remaining,batch,merged,family){
  const FAM=merged?'':' '+String(family||'').toUpperCase();
  if(!total)return emptyShareLabel(merged,family);
...
/* V235 — une photo sans famille n'appartient plus ni à BRUN ni à BLANC : reprise dans
   ...
   donc toutes les familles.
   V255.1 — la Sortie magasin rend compte d'UNE visite : seules les photos qui portent
   son `visitId` sont partagées, et ce sont les mêmes que comptent le compte rendu local
   et la charge IA. Les photos des visites précédentes du magasin restent dans la
   galerie mais ne partent plus. Une ancienne photo sans `visitId` n'est jamais rattachée
   par déduction (même jour, même magasin) : elle ne rejoint pas le partage d'une visite
   dont rien ne prouve qu'elle vient. Lecture seule, aucun enregistrement réécrit. */
