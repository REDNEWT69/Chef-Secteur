...
(async()=>{
  ...
  const nord=t.sites.find(s=>s.city==='TEST-NORD');assert.equal(nord.activeContract.status,'En cours');assert.equal(nord.activeContract.objective,7200);assert.equal(nord.history.length,2);assert.equal(nord.activeContract.sector,'Zone Ancienne');
