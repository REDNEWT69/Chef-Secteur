...
const hitRows=[[],[],[],[],[],[],[null,'Secteur 2026','Magasin physique (lib)'],[null,'Autre','SCH-HORS-ZONE FR-75000'],[null,'Auvergne-Rhone-Alpes','SCH-SAINT-PRIEST FR-69800'],[null,null,'SCH-ST PAUL LES ROMANS E FR-26750'],[null,null,'SCH-VILLE-LA-GRAND FR-74100'],[null,'Total Auvergne-Rhone-Alpes']];
...
const trackingRows=[H,row('Valence','SCHMIDT','SAINT-PRIEST',111,'Finalisé',6000,7000,1.1,'Fin','gratuit','OLDREF',45597,46081),row('Valence','SCHMIDT','SAINT-PRIEST',111,'En cours',7200,6300,.875,6,'25%','BRBTEST',46082,46446),row('Auvergne-Rhone-Alpes','SCHMIDT','SAINT-PAUL-LES-ROMANS',222,'Alerte',5400,2200,.407,1,'100%','MICROTEST',45930,46300),row('Valence','SCHMIDT','VILLE-LA-GRAND',333,'Finalisé',8000,9000,1.125,'Fin','gratuit','HISTREF',45200,45930),row('Bordeaux','SCHMIDT','HORS-ZONE',444,'Alerte',1,0,0,1,'100%','NOPE')];
const tracking=workbook([{name:'HITLIST SECTEUR (VALEURS)',rows:hitRows},{name:'Suivi contrats exposition',rows:trackingRows}]);
...
global.state={stores:[{id:'s1',enseigne:'Schmidt',ville:'Saint-Priest',priority:4},{id:'s2',enseigne:'Schmidt',ville:'Saint-Paul-les-Romans',priority:3},{id:'s3',enseigne:'Schmidt',ville:'Ville-la-Grand',priority:2}],plan:{Lundi:[]},businessV2:{visits:[],actions:[],storeSnapshots:{}}};
...
(async()=>{
  ...
  assert.equal(t.sector,'Auvergne-Rhone-Alpes');assert.equal(t.sites.length,3);assert(!t.sites.some(s=>/HORS-ZONE/.test(s.key)));
  const sp=t.sites.find(s=>s.city==='SAINT-PRIEST');assert.equal(sp.activeContract.status,'En cours');assert.equal(sp.activeContract.objective,7200);assert.equal(sp.history.length,2);assert.equal(sp.activeContract.sector,'Valence');
  const romans=t.sites.find(s=>/ROMANS/.test(s.city));assert.equal(romans.activeContract.status,'Alerte');assert.equal(romans.activeContract.monthsRemaining,1);
  const ville=t.sites.find(s=>s.city==='VILLE-LA-GRAND');assert.equal(ville.activeContract,null);assert.equal(ville.lastContract.status,'Finalisé');
  ...
  const ctx=api.compactContext({planning:{safe:true}});assert.equal(ctx.cuisinistesV193.sector,'Auvergne-Rhone-Alpes');assert(ctx.cuisinistesV193.sites.length<=10);assert(!JSON.stringify(ctx).includes('HITLIST SECTEUR'));
  const ans=api.answer('Prépare-moi ma visite chez Saint-Priest');assert(/point chiffre/i.test(ans));assert(/6 mois/.test(ans));
  assert.equal(JSON.stringify(global.state),before,'questions/import must not mutate planning or store.priority');
  console.log('V193 cuisinistes: OK · 3 Rhône-Alpes · Valence legacy ignored · alert/history/tariff/assistant');
