...
assert(index.includes("const BUILD_REV='20260916-inlinestr191'"),'index doit publier le build V191');
assert(sw.includes('const BUILD_REV = "20260916-inlinestr191"'),'sw doit publier le même build V191');
assert.equal(version.latestBuild,'20260916-inlinestr191');
assert.equal(version.displayVersion,'191');
...
assert(index.includes('<img src="./app-icon.svg?rev=20260916-inlinestr191" alt="S-RUNNER">'),'le loader doit afficher le vrai logo S-RUNNER');
