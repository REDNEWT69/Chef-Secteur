...
assert(index.includes("const BUILD_REV='20260916-aistore194'"),'index doit publier le build V194');
assert(sw.includes('const BUILD_REV = "20260916-aistore194"'),'sw doit publier le même build V194');
assert.equal(version.latestBuild,'20260916-aistore194');
assert.equal(version.displayVersion,'194');
...
assert(index.includes('<img src="./app-icon.svg?rev=20260916-aistore194" alt="S-RUNNER">'),'le loader doit afficher le vrai logo S-RUNNER');
