...
assert(index.includes("const BUILD_REV='20260917-daytabs208'"),'index doit publier le build V208');
assert(sw.includes('const BUILD_REV = "20260917-daytabs208"'),'sw doit publier le même build V208');
assert.equal(version.latestBuild,'20260917-daytabs208');
assert.equal(version.displayVersion,'208');
...
assert(index.includes('<img src="./app-icon.svg?rev=20260917-daytabs208" alt="S-RUNNER">'),'le loader doit afficher le vrai logo S-RUNNER');
