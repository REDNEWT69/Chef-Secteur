...
assert(index.includes("const BUILD_REV='20260917-opportunity200'"),'index doit publier le build V200');
assert(sw.includes('const BUILD_REV = "20260917-opportunity200"'),'sw doit publier le même build V200');
assert.equal(version.latestBuild,'20260917-opportunity200');
assert.equal(version.displayVersion,'200');
...
assert(index.includes('<img src="./app-icon.svg?rev=20260917-opportunity200" alt="S-RUNNER">'),'le loader doit afficher le vrai logo S-RUNNER');
