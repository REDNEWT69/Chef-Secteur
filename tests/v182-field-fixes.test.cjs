...
assert(index.includes("const BUILD_REV='20260916-perfassist192'"),'index doit publier le build V192');
assert(sw.includes('const BUILD_REV = "20260916-perfassist192"'),'sw doit publier le même build V192');
assert.equal(version.latestBuild,'20260916-perfassist192');
assert.equal(version.displayVersion,'192');
...
assert(index.includes('<img src="./app-icon.svg?rev=20260916-perfassist192" alt="S-RUNNER">'),'le loader doit afficher le vrai logo S-RUNNER');
