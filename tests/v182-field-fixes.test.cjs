...
assert(index.includes("const BUILD_REV='20260916-performance190'"),'index doit publier le build V190');
assert(sw.includes('const BUILD_REV = "20260916-performance190"'),'sw doit publier le même build V190');
assert.equal(version.latestBuild,'20260916-performance190');
assert.equal(version.displayVersion,'190');
...
assert(index.includes('<img src="./app-icon.svg?rev=20260916-performance190" alt="S-RUNNER">'),'le loader doit afficher le vrai logo S-RUNNER');
