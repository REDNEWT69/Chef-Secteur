...
const stores=[
 {id:'aub',enseigne:'Boulanger',sourceName:'BOULANGER AUBIERE / Clermont',ville:'Aubière',x:-150,y:0},
 {id:'ann',enseigne:'Boulanger',sourceName:'BOULANGER ANNEMASSE',ville:'Annemasse',x:150,y:20},
 {id:'cha',enseigne:'Boulanger',sourceName:'BOULANGER CHAMBERY',ville:'Chambéry',x:100,y:0},
 {id:'val',enseigne:'Boulanger',sourceName:'BOULANGER VALENCE',ville:'Valence',x:0,y:-95},
 {id:'vil',enseigne:'Boulanger',sourceName:'BOULANGER VILLARS / St Etienne',ville:'Villars',x:-55,y:-20},
 {id:'auc',enseigne:'Auchan',sourceName:'AUCHAN SAINT PRIEST',ville:'Saint-Priest',x:15,y:0},
 {id:'but',enseigne:'BUT',sourceName:'BUT LYON SAINT PRIEST',ville:'Saint-Priest',x:15,y:0}
];
const state={
...
assert.equal(api.completionDate(api.campaign.targets.find(t=>t.key==='auchan-saint-priest')),'2026-09-14');
assert.equal(api.completionDate(api.campaign.targets.find(t=>t.key==='but-saint-priest')),'2026-09-15');
...
assert.deepEqual(new Set([result.overnight.fromStore.id,result.overnight.toStore.id]),new Set(['cha','ann']),'Chambéry et Annemasse doivent former le bloc de découché');
...
for(const id of ['aub','ann','cha','val','vil'])assert.ok(dates[id]>='2026-09-17'&&dates[id]<='2026-09-22',id+' doit être planifié du 17 au 22');
assert.ok(!Object.values(plans).flatMap(p=>Object.values(p)).flat().some(s=>s.id==='auc'||s.id==='but'));
