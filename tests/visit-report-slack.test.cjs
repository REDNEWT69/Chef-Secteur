...
// Les comptes rendus fusionnés gardent un CR unique et reprennent toutes les photos du magasin.
(function merged(){for(const [enseigne,titre] of [['Schmidt','# COMPTE RENDU DE VISITE — CUISINISTE'],['Gitem','# COMPTE RENDU DE VISITE — BUYING GROUP']]){const {s,id}=freshVisit(enseigne);M.editReport(s,id,'blanc','team','Note blanc.');M.editReport(s,id,'brun','team','Note brun.');const photos=[{family:'blanc',moment:'avant'},{family:'brun',moment:'apres'},{family:'',moment:''}],txt=R.build(s,id,'brun',photos);assert.ok(txt.startsWith(titre));assert.ok(!txt.includes('Famille '));assert.ok(txt.includes('Note blanc.\n\nNote brun.'));assert.ok(txt.includes('> **Photos :** 1 avant / 1 après / 1 sans moment jointes à ce message.'));assert.equal(R.build(s,id,'blanc',photos),txt);const payload=R.buildAIPayload(s,id,'brun',photos);assert.equal(payload.photos.total,3);assert.equal(payload.photos.before,1);assert.equal(payload.photos.after,1);assert.equal(payload.photos.other,1);const prompt=R.aiPrompt(payload);assert.ok(prompt.includes('N’invente aucun fait'))}})();
...
(function uiSimple(){const src=fs.readFileSync(__dirname+'/../store-runner-visits.js','utf8');assert.ok(src.includes("const VISIBLE_STEPS=[3]"),'seul le carnet Terrain reste visible');assert.ok(!src.includes('Méthode 6P'),'aucune étape 6P visible');assert.ok(!src.includes('function sixP('),'aucun formulaire 6P rendu');assert.ok(src.includes('Famille active : '),'la famille active est explicitement affichée');assert.ok(src.includes("'Note terrain '+family.toUpperCase()"));assert.ok(src.includes("'Prochain passage / formation '+family.toUpperCase()"));assert.ok(src.includes('api.open(v.storeId,{visitId:v.id,family:shownFamily(v)})'),'les photos sont accessibles directement depuis le carnet, ouvertes sur la visite et sa famille')})();

(function proprete(){const src=fs.readFileSync(__dirname+'/../visit-report-slack.js','utf8');assert.ok(!/new MutationObserver/.test(src));assert.ok(!/setInterval\s*\(/.test(src));assert.ok(!/\bfetch\s*\(|XMLHttpRequest/.test(src),'la sortie magasin réutilise la passerelle existante sans accès réseau direct');assert.ok(/store-runner:data-restored/.test(src)&&/store-runner:planning-updated/.test(src));assert.ok(src.includes('root.callAIGateway'));assert.ok(src.includes('✨ Générer le résumé '));assert.ok(src.includes('Modifier le texte'));assert.ok(src.includes("merged&&typeof api.list==='function'"),'un CR fusionné charge toutes les familles, puis filtre sur la visite');assert.ok(src.includes('api.list(storeId)'),'le CR fusionné utilise la liste magasin complète');assert.ok(src.includes('api.listByFamily(storeId,family)'),'un store-photos plus ancien en cache reste lisible, refiltre par la Sortie magasin');assert.ok(src.includes('api.shareRecords(batch)'),'V235 : seul le lot courant part au partage systeme');assert.ok(!src.includes('api.shareRecords(rows)'),'V235 : la totalite des photos ne part plus en un seul appel');assert.ok(src.includes("api.listStrictByFamily(storeId,family)"),'V235 : la Sortie magasin lit la famille en strict, sans les photos non classees');assert.ok(!src.includes('b.hidden=!!merged'),'le bouton de partage ne doit plus disparaître pour les CR fusionnés');assert.ok(src.includes("merged?'Aucune photo pour cette visite.'"),'V255.1 : un CR fusionné vide parle de la visite, plus du magasin');assert.ok(src.includes("'Aucune photo '+String(family||'').toUpperCase()+' pour cette visite.'"),'V255.1 : une famille vide parle de la visite');assert.ok(src.includes('return ofVisit(rows,visitId)'),'V255.1 : seules les photos de la visite partent');assert.ok(src.includes('Partage annulé.'))})();

console.log('PASS: carnet terrain simple, CR local hors ligne, génération IA FMT validée et photos par famille / CR fusionné.');
