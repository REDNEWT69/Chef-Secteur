...
assert(index.includes("'./range-planner-v2.js','./terrain-planning-v1.js','./working-hours-end.js'"), 'le module terrain doit charger juste après le range planner');
assert(sw.includes('"./terrain-planning-v1.js"'), 'le module terrain doit être disponible hors ligne');
assert.strictEqual(version.displayVersion, '153');
assert.strictEqual(version.latestBuild, '20260913-terrain153');
assert(index.includes("const BUILD_REV='20260913-terrain153'"));
assert(sw.includes('const BUILD_REV = "20260913-terrain153"'));
