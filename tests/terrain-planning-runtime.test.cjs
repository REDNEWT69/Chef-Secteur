...
assert.strictEqual(version.displayVersion, '178');
assert.strictEqual(version.latestBuild, '20260915-weeklytarget178');
assert(index.includes("const BUILD_REV='20260915-weeklytarget178'"));
assert(sw.includes('const BUILD_REV = "20260915-weeklytarget178"'));
...
assert(index.includes("'./daily-capacity.js','./weekly-target.js','./planning-pro-plus.js'"), 'la persistance de l’objectif hebdomadaire doit charger avec les réglages de capacité');
