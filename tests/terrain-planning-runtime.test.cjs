...
assert.strictEqual(version.displayVersion, '187');
assert.strictEqual(version.latestBuild, '20260916-priority187');
assert(index.includes("const BUILD_REV='20260916-priority187'"));
assert(sw.includes('const BUILD_REV = "20260916-priority187"'));
...
assert(index.includes("'./sector-pilotage.js','./v182-fixes.js','./priority-campaign-v187.js','./auto-planning-fix.js'"), 'la campagne V187 doit charger après les règles géographiques et avant la finalisation runtime');
assert(sw.includes('"./v182-fixes.js"'), 'les correctifs V182/V185 doivent rester disponibles hors ligne');
assert(sw.includes('"./priority-campaign-v187.js"'), 'les priorités services V187 doivent rester disponibles hors ligne');
