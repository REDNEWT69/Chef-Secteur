...
assert(index.includes("'./range-planner-v2.js','./store-opening-hours.js','./boulanger-default-hours.js','./terrain-planning-v1.js','./working-hours-end.js'"), 'le défaut Boulanger doit charger entre les horaires et le module terrain');
assert(sw.includes('"./terrain-planning-v1.js"'), 'le module terrain doit être disponible hors ligne');
assert.strictEqual(version.displayVersion, '163');
assert.strictEqual(version.latestBuild, '20260913-boulangerhours163');
assert(index.includes("const BUILD_REV='20260913-boulangerhours163'"));
assert(sw.includes('const BUILD_REV = "20260913-boulangerhours163"'));
assert(sw.includes('"./store-opening-hours.js"'), 'horaires disponibles hors ligne');
assert(sw.includes('"./boulanger-default-hours.js"'), 'défaut Boulanger disponible hors ligne');
