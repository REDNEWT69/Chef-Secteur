...
const rows=R.parse(response([node(1),{...node(2),type:'way',lat:undefined,lon:undefined,center:{lat:45.75,lon:4.85}},node(3,{brand:'Darty'}),node(4,{shop:'vacant'}),node(5,{disused:'yes'})]),region,['Boulanger']);
...
assert.throws(()=>R.parse({elements:[],remark:'timeout'},region,['Boulanger']));assert.throws(()=>R.parse({elements:[]},region,['Boulanger']));
assert.deepEqual(R.parse(response([]),region,['Boulanger']),[]);
const missing=R.parse(response([{...node(10),lat:undefined,lon:undefined,tags:{shop:'electronics',brand:'Boulanger'}}]),region,['Boulanger'])[0];assert(missing.needsGeo);assert.equal(missing.ville,'');assert(!R.coords({lat:null,lon:null}));
