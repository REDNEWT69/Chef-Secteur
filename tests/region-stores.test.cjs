...
const node=(id,extra={})=>({type:'node',id,lat:43.65,lon:-0.65,tags:{shop:'electronics',brand:'Boulanger','addr:city':'Ville-Test A','addr:street':'Rue Test',...extra}});
...
const rows=R.parseOSM(response([node(1),{...node(2),type:'way',lat:undefined,lon:undefined,center:{lat:43.65,lon:-0.65}},node(3,{brand:'Darty'}),node(4,{shop:'vacant'}),node(5,{disused:'yes'})]),region,['Boulanger']);
...
assert(R.duplicate(rows[0],[{...rows[0],id:'old',lat:43.6501}]));assert(!R.duplicate(rows[0],[{...rows[0],id:'far',lat:43.9,adresse:'Autre rue'}]));
...
const fail=global.localStorage.setItem;global.localStorage.setItem=()=>{throw Error('quota')};assert.throws(()=>R.commit([{...rows[0],id:'new',lat:45.9,adresse:'Different'}]));assert.equal(JSON.stringify(global.state),before);global.localStorage.setItem=fail;
...
for(const tags of [
 ...
 {shop:'electronics',brand:'Autre marque',name:'Boulanger Ville-Test A'},
 {shop:'convenience',brand:'Carrefour',name:'Carrefour City'}
])assert.equal(R.matchBrand(tags,R.BRANDS),undefined);
assert.equal(R.matchBrand({shop:'electronics',name:'Boulanger Ville-Test A'},R.BRANDS),'Boulanger');
assert.equal(R.matchBrand({shop:'electronics',brand:'Boulanger',name:'Boulanger Ville-Test A'},R.BRANDS),'Boulanger');
assert.throws(()=>R.commit([{...rows[0],id:'no-address',adresse:'',lat:45.9}]));
