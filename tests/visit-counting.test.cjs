...
assert.equal(V.credit({enseigne:'Carrefour'}),1,'une enseigne non confirmée reste à une visite');
...
assert.equal(V.planCredits(state.plan),5,'Darty + Boulanger + Carrefour doivent produire cinq visites comptabilisées');
...
assert.equal(month.visits,5,'septembre doit compter cinq visites métier');
assert.doesNotMatch(source,/visitMinutes\s*=/,'le double comptage ne doit jamais doubler la durée de visite');
assert.match(source,/darty:2,boulanger:2/,'les deux enseignes confirmées doivent être les seules règles par défaut');
console.log('PASS: Darty et Boulanger comptent chacun pour deux visites métier, les autres restent à une visite, les arrêts physiques et durées restent séparés, et les règles sont extensibles.');
