...
/*
 * V189 garde l'application automatique des propositions et ajoute deux contrôles
 * terrain qui appartiennent au chef de secteur : le poids métier d'un magasin et
 * ses familles produits. Le même runtime fiabilise aussi le découché : jamais dans
 * le passé, badge visible sur le bon jour et recherche d'hôtel réellement exploitable.
 */
...
function restoreCreditOverrides(){
  ...
  for(const store of stores()){
    ...
    if(own===1||own===2)continue;
    const saved=Number(map[storeFingerprint(store)]);
    if(saved===1||saved===2){store.visitCreditOverride=saved;changed=true}
  }
  return changed
}
...
function configuredRules(){
  ...
  const map=Object.assign({},DEFAULT_CREDITS,configured);map.carrefour=1;return map
}
...
function visitCredit(store){
  if(!store)return 0;const own=Number(store.visitCreditOverride);
  return own===1||own===2?own:brandCredit(store)
}
function planningCapacityActive(){
  try{if(window.__storeRunnerPlanningGenerationActive)return true;const b=document.getElementById('generateRangeBtn');return !!(b&&b.disabled)}catch(e){return false}
}
...
function archiveStats(archive,start,end){
  ...
  for(const [key,snap] of Object.entries(archive||{})){
    ...
    for(let i=0;i<DAYS.length;i++){
      ...
      for(const s of (snap.plan[DAYS[i]]||[])){count++;visits+=visitCredit(s);unique.add(storeKey(s))}
    }
  }
...
function normalizeCandidate(candidate){
  ...
  candidate.storeCount=planStores(candidate.plan);candidate.visitCredits=planCredits(candidate.plan);
  if(candidate.range){
    ...
    candidate.range.totalStores=stats.stores;candidate.range.totalVisits=stats.visits;candidate.range.uniqueStores=stats.uniqueStores||candidate.range.uniqueStores||0;candidate.range.visitCreditRules=configuredRules()
  }
  return candidate
}
function reconcileStoredRangeStats(){
  const db=storage();if(!db)return null;let archive={},range=null;
  try{archive=JSON.parse(db.getItem('chef_sector_plan_archive_v1')||'{}')||{};range=JSON.parse(db.getItem('chef_sector_range_v1')||'null')}catch(e){return null}
  if(!range||!range.start||!range.end)return null;const stats=archiveStats(archive,range.start,range.end);
  range.totalStores=stats.stores;range.totalVisits=stats.visits;range.uniqueStores=stats.uniqueStores;range.visitCreditRules=configuredRules();range.updatedAt=new Date().toISOString();
  try{db.setItem('chef_sector_range_v1',JSON.stringify(range));if(typeof db.flush==='function'){const p=db.flush();if(p&&typeof p.catch==='function')p.catch(()=>{})}}catch(e){return null}
  return range
}
function monthArchiveStats(year,month){
  const db=storage();let archive={};try{archive=db?JSON.parse(db.getItem('chef_sector_plan_archive_v1')||'{}')||{}:{}}catch(e){}
  let count=0,visits=0;const unique=new Set();
  for(const [key,snap] of Object.entries(archive)){
    const mon=parse((snap&&snap.weekMonday)||key);if(!mon||!snap||!snap.plan)continue;
    for(let i=0;i<DAYS.length;i++){
      const date=addDays(mon,i);if(date.getFullYear()!==year||date.getMonth()!==month)continue;
      for(const s of (snap.plan[DAYS[i]]||[])){count++;visits+=visitCredit(s);unique.add(storeKey(s))}
    }
  }
  return{stores:count,visits,uniqueStores:unique.size}
...
function patchCountingApi(){
  ensureCarrefourDefault();restoreCreditOverrides();
  window.storeVisitCredit=planningCredit;window.storeVisitCreditsForRoute=routeCredits;window.storeVisitCreditsForPlan=planCredits;window.storeVisitStoresForPlan=planStores;
  const V=window.StoreVisitCounting;if(V){
    V.credit=visitCredit;V.planningCredit=planningCredit;V.routeCredits=routeCredits;V.planCredits=planCredits;V.planStores=planStores;V.archiveStats=archiveStats;V.normalizeCandidate=normalizeCandidate;V.reconcileStoredRangeStats=reconcileStoredRangeStats;V.monthArchiveStats=monthArchiveStats;V.rules=configuredRules
  }
  return true
}

function ensureStoreRulesUi(){
  const dlg=document.getElementById('storeDlg'),note=document.getElementById('fNote');if(!dlg||!note)return false;
  if(document.getElementById('srStoreRulesV189'))return true;
...
function fillStoreRules(id){
  ensureStoreRulesUi();const store=id?findStore(id):null,select=document.getElementById('fVisitCreditOverride');
  if(select)select.value=String(store?visitCredit(store):1)
}
function patchStoreFunctions(){
  if(typeof window.openStore==='function'&&!window.openStore.__v189StoreRules){
    const original=window.openStore;const wrapped=function(id){ensureStoreRulesUi();restoreCreditOverrides();const out=original.apply(this,arguments);fillStoreRules(id);return out};wrapped.__v189StoreRules=true;wrapped.__original=original;window.openStore=wrapped
  }
  if(typeof window.saveStore==='function'&&!window.saveStore.__v189StoreRules){
    const original=window.saveStore;const wrapped=function(){
      ensureStoreRulesUi();const select=document.getElementById('fVisitCreditOverride'),credit=select&&Number(select.value)===2?2:1,before=new Set(stores().map(s=>String(s.id))),editing=window.currentEditId==null?'':String(window.currentEditId);
      const out=original.apply(this,arguments);let store=editing?findStore(editing):stores().find(s=>!before.has(String(s.id)))||null;
      if(store){saveCreditOverride(store,credit);try{if(typeof window.save==='function')window.save()}catch(e){}try{if(typeof window.renderStores==='function')window.renderStores()}catch(e){}refreshCountingUi()}
      return out
    };wrapped.__v189StoreRules=true;wrapped.__original=original;window.saveStore=wrapped
  }
  return true
}

function productsLabel(store){const p=(store&&Array.isArray(store.products)?store.products:[]).filter(x=>x&&x!=='À confirmer');return p.join(' + ')}
function refreshCountingUi(){
  ...
  const summary=document.getElementById('summary');if(summary){const spans=summary.querySelectorAll('span');if(spans[0])spans[0].textContent=physical+' magasins planifiés';if(spans[1])spans[1].textContent=visits+' visites comptabilisées'}
...
function selectedWeekMonday(){const d=parse(window.state&&state.settings&&state.settings.weekDate)||new Date();return monday(d)}
function futureOvernightAnalysis(plan){
  ...
  for(let i=0;i<days.length-1;i++){
    ...
    if(Math.round((parse(toDate)-parse(fromDate))/86400000)!==1)continue;
    const a=source[fromDay]||[],b=source[toDay]||[];if(!a.length||!b.length)continue;
    const last=a[a.length-1],first=b[0],home1=homeDistance(last),home2=homeDistance(first),direct=distance(last,first),saving=home1+home2-direct;if(!Number.isFinite(saving))continue;
    const row={night:'Nuit '+fromDay+' → '+toDay,fromDay,toDay,fromDate,toDate,last,first,saving,fromHome:home1,toHome:home2,remoteKm:Math.min(home1,home2)};
    if(!best||row.saving>best.saving)best=row;
    if(row.remoteKm>=REMOTE_MIN_KM&&(!bestRemote||row.saving>bestRemote.saving))bestRemote=row;
    if(row.remoteKm>=REMOTE_MIN_KM&&row.saving>=MIN_USEFUL_OVERNIGHT_KM&&(!bestUseful||row.saving>bestUseful.saving))bestUseful=row
  }
...
function decorateOvernightDay(candidate){
  ensureOvernightStyle();document.querySelectorAll('.srOvernightStarV189').forEach(e=>e.remove());document.querySelectorAll('.srOvernightDayV189').forEach(e=>e.classList.remove('srOvernightDayV189'));
  if(!candidate)return false;const cards=document.querySelectorAll('#week .day');for(const card of cards){const title=card.querySelector('.dayhead b');if(!title||norm(title.textContent)!==norm(candidate.fromDay))continue;card.classList.add('srOvernightDayV189');const head=card.querySelector('.dayhead>div')||card.querySelector('.dayhead');if(head){const badge=document.createElement('span');badge.className='srOvernightStarV189';badge.textContent='★ Nuit sur place';head.appendChild(badge)}return true}return false
}
function renderOvernightV189(){
  ...
  if(a.reason==='disabled'){box.innerHTML='<div class="notice">🌙 Découché désactivé dans Secteur.</div>';return true}
  if(!o){box.innerHTML='<div class="notice">🌙 Aucun découché futur utile retenu avec le planning actuel. Les nuits déjà passées sont ignorées.</div>';return true}
...
function patchOvernight(){
  ensureOvernightStyle();
  const candidate=function(){return futureOvernightAnalysis().candidate};candidate.__v182Wrapped=true;candidate.__v189FutureOnly=true;window.overnightCandidate=candidate;
  const render=function(){return renderOvernightV189()};render.__v182Wrapped=true;render.__v189FutureOnly=true;window.renderOvernight=render;
  if(window.StoreRunnerOvernightV182)window.StoreRunnerOvernightV182.analyze=futureOvernightAnalysis;
  if(typeof window.renderWeek==='function'&&!window.renderWeek.__v189Overnight){const original=window.renderWeek;const wrapped=function(){const out=original.apply(this,arguments);renderOvernightV189();return out};wrapped.__v189Overnight=true;wrapped.__original=original;window.renderWeek=wrapped}
  renderOvernightV189();return true
}
...
function installAutoApply(){
  ...
  var fn=async function(candidate){
    try{
      ...
      R.persist(bundle);window.state=bundle.state;restoreCreditOverrides();ensureCarrefourDefault();
      if(typeof window.initControls==='function')window.initControls();
      if(typeof window.renderAll==='function')window.renderAll();
      refreshCountingUi();patchOvernight();
      return true;
    }catch(e){
...
function patchRenderAll(){
  if(typeof window.renderAll==='function'&&!window.renderAll.__v189Restore){const original=window.renderAll;const wrapped=function(){restoreCreditOverrides();ensureCarrefourDefault();const out=original.apply(this,arguments);refreshCountingUi();renderOvernightV189();return out};wrapped.__v189Restore=true;wrapped.__original=original;window.renderAll=wrapped}
}
function repair(){
  if(!window.state){if(window.setTimeout&&bootRetries++<40)window.setTimeout(repair,100);return false}
  const changed=ensureCarrefourDefault()|restoreCreditOverrides();patchCountingApi();ensureStoreRulesUi();patchStoreFunctions();patchRenderAll();patchOvernight();installAutoApply();refreshCountingUi();
  if(changed){try{if(typeof window.save==='function')window.save()}catch(e){}}
  return true
}
...
document.addEventListener('store-runner:data-restored',()=>window.setTimeout(repair,0));
document.addEventListener('store-runner:planning-updated',()=>window.setTimeout(()=>{repair();refreshCountingUi();renderOvernightV189()},0));
document.addEventListener('store-runner:home-rendered',()=>window.setTimeout(()=>{refreshCountingUi();decorateOvernightDay(futureOvernightAnalysis().candidate)},0));
window.addEventListener('load',()=>window.setTimeout(repair,0),{once:true});
[150,600,1400].forEach(ms=>window.setTimeout(repair,ms));
