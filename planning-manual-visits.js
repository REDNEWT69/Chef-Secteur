/* Store Runner V1 — modifications manuelles rapides du planning. */
(function(root,factory){
  const api=factory();
  if(typeof module!=='undefined'&&module.exports)module.exports=api;
  if(root&&root.document){root.StoreRunnerManualPlanning=api;api.install(root)}
...
function storage(win){try{return win.__chefStorage||win.localStorage||null}catch(e){return win.__chefStorage||null}}
function cloneStore(s){return{id:s.id||'',enseigne:s.enseigne||'',ville:s.ville||'',adresse:s.adresse||'',dept:s.dept||'',lat:s.lat,lon:s.lon,freq:s.freq||'',priority:s.priority,lastVisit:s.lastVisit||'',intervalDays:s.intervalDays,products:Array.isArray(s.products)?s.products.slice():s.products}}
function clonePlan(state){const p={};for(const d of DAYS)p[d]=((state&&state.plan&&state.plan[d])||[]).map(cloneStore);return p}
function currentWeekKey(state){const raw=parse(state&&state.settings&&state.settings.weekDate)||new Date();return iso(monday(raw))}
function currentDay(win){const active=win.document.querySelector('#dayTabs .periodDayTab.active[data-date]');if(active){const d=parse(active.dataset.date);if(d)return dayName(d)}const old=win.document.querySelector('#dayTabs .dayTab.active');if(old){const attr=old.getAttribute&&old.getAttribute('onclick'),m=String(attr||'').match(/selectPlanningDay\(['"]([^'"]+)/);if(m&&DAYS.includes(m[1]))return m[1]}if(DAYS.includes(win.selectedPlanningDay))return win.selectedPlanningDay;const days=(win.state&&win.state.settings&&win.state.settings.days)||DAYS;return days[0]||'Lundi'}
function findStore(state,id){return ((state&&state.stores)||[]).find(s=>String(s.id)===String(id))||null}
function plannedDay(state,id){for(const d of DAYS)if(((state&&state.plan&&state.plan[d])||[]).some(s=>String(s.id)===String(id)))return d;return''}
function addToPlan(state,id,day){if(!state||!DAYS.includes(day))return{ok:false,error:'Jour invalide'};const store=findStore(state,id);if(!store||store.active===false||(state.excluded&&state.excluded[id]))return{ok:false,error:'Magasin indisponible'};state.plan=state.plan||{};for(const d of DAYS)if(!Array.isArray(state.plan[d]))state.plan[d]=[];const sourceDay=plannedDay(state,id);if(sourceDay===day)return{ok:true,already:true,store,sourceDay,targetDay:day};if(sourceDay)state.plan[sourceDay]=state.plan[sourceDay].filter(s=>String(s.id)!==String(id));if(!state.plan[day].some(s=>String(s.id)===String(id)))state.plan[day].push(store);return{ok:true,store,sourceDay,targetDay:day}}
function removeFromPlan(state,id,day){if(!state||!DAYS.includes(day)||!state.plan||!Array.isArray(state.plan[day]))return{ok:false};const before=state.plan[day].length,store=findStore(state,id)||state.plan[day].find(s=>String(s.id)===String(id))||null;state.plan[day]=state.plan[day].filter(s=>String(s.id)!==String(id));return{ok:state.plan[day].length<before,store,day}}
