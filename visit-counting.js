...
function patchSummary(){
  ...
  el.innerHTML='<span>'+stats.stores+' magasins planifiés</span><span>'+stats.visits+' visites comptabilisées</span><span>~'+Math.round(km)+' km géographiques</span><span>'+visitedVisits+' visites cochées aujourd’hui'+(visitedStores&&visitedVisits!==visitedStores?' · '+visitedStores+' magasins':'')+'</span>';
}
function patchLegacyBrief(){
  ...
  const stats=currentPlanStats();value.textContent=stats.visits+' visites';if(sub)sub.textContent=stats.stores+' magasins planifiés · objectif '+Number((state.settings&&state.settings.target)||20)+' magasins';
}
function patchPremiumHome(){
  const card=document.querySelector('#premiumHomeV2 .phGrid .phCard:first-child');if(!card)return;
  const value=card.querySelector('.phValue'),sub=card.querySelector('.phSub'),stats=currentPlanStats();if(value)value.textContent=stats.visits+' visites';if(sub)sub.textContent=stats.stores+' magasins planifiés · objectif '+Number((state.settings&&state.settings.target)||20)+' magasins';
}
...
function patchProMonth(){
  const month=visibleMonth();if(!month)return;const stats=monthArchiveStats(month.year,month.month),head=document.querySelector('#proMonthBody .proMonthHead span');if(head)head.textContent=stats.visits+' visites comptabilisées · '+stats.uniqueStores+' magasins distincts';const first=document.querySelector('#proMonthMetrics > div:first-child');if(first){const b=first.querySelector('b'),s=first.querySelector('span');if(b)b.textContent=String(stats.visits);if(s)s.textContent='visites comptabilisées'}document.querySelectorAll('#proMonthBody .proMore').forEach(e=>{e.textContent=e.textContent.replace(/visites?/i,'magasins')});
}
function patchQuickStore(){
  const sheet=document.getElementById('storeQuickSheet'),start=document.getElementById('srQuickStart');if(!sheet||!start||!start.dataset)return;const id=start.dataset.srStart;if(!id)return;let store=null;try{store=(state.stores||[]).find(s=>String(s.id)===String(id))}catch(e){}if(!store)return;const address=document.getElementById('sqAddress');if(!address)return;let badge=document.getElementById('sqVisitCredit');if(!badge){badge=document.createElement('div');badge.id='sqVisitCredit';badge.className='tiny';badge.style.marginTop='6px';address.insertAdjacentElement('afterend',badge)}const c=visitCredit(store);badge.textContent=c>1?'Ce passage compte pour '+c+' visites (Blanc + Brun).':'Ce passage compte pour 1 visite.';
}
...
function patchRangeStatus(candidate){
  const el=document.getElementById('rangePlanStatus');if(!el||!candidate)return;const stores=Number(candidate.range&&candidate.range.totalStores)||Number(candidate.storeCount)||0,visits=Number(candidate.range&&candidate.range.totalVisits)||Number(candidate.visitCredits)||0;if(candidate.range)el.textContent='Période appliquée : '+candidate.range.weeks+' semaines · '+stores+' magasins · '+visits+' visites comptabilisées · '+Number(candidate.range.uniqueStores||0)+' magasins distincts.';else el.textContent='Semaine générée : '+stores+' magasins · '+visits+' visites comptabilisées.';
}
...
function hookRenderers(){
  if(typeof window.renderAll==='function'&&!window.renderAll.__visitCreditsWrapped){const original=window.renderAll;const wrapped=function(){const out=original.apply(this,arguments);schedulePatch();return out};wrapped.__visitCreditsWrapped=true;window.renderAll=wrapped}
  if(typeof window.openStoreQuick==='function'&&!window.openStoreQuick.__visitCreditsWrapped){const original=window.openStoreQuick;const wrapped=function(){const out=original.apply(this,arguments);schedulePatch();return out};wrapped.__visitCreditsWrapped=true;window.openStoreQuick=wrapped}
  window.assistantSummary=formatAssistantSummary;
}
function boot(){ensureRules();hookRenderers();hookReliability();schedulePatch();document.addEventListener('store-runner:planning-updated',schedulePatch);document.addEventListener('store-runner:data-restored',()=>{ensureRules();schedulePatch()});document.addEventListener('chef-range-generated',schedulePatch);document.addEventListener('click',e=>{if(e.target&&e.target.closest&&e.target.closest('.proMonthPrev,.proMonthNext'))schedulePatch()});document.addEventListener('touchend',e=>{if(e.target&&e.target.closest&&e.target.closest('#proMonthBody'))schedulePatch()},{passive:true})}
