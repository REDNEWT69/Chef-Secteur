...
function install(){
 ...
 ensureStyle();
 const box=document.createElement('section');box.id='backupTools';box.className='card';box.innerHTML='<h2>Sauvegardes</h2><p>Les copies automatiques restent sur ce téléphone. Exporte aussi un fichier pour pouvoir récupérer tes données sur un autre appareil. Les photos s’exportent à part, depuis la galerie photo.</p><button class="primary" id="downloadBackup" type="button">Sauvegarder mes données</button><label>Restaurer un fichier JSON<input id="restoreBackupFile" type="file" accept=".json,application/json"></label><p id="backupMeta"></p><p id="backupFeedback" role="status"></p><div id="storageHealth"></div><details><summary>Versions automatiques (8 maximum)</summary><div id="backupVersions"></div></details>';
