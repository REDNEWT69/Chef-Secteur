...
function refresh(){const meta=document.getElementById('backupMeta'),list=document.getElementById('backupVersions');if(!meta||!list)return;
 try{const exported=localStorage.getItem(R.keys.EXPORT);meta.textContent=exported?'Dernier export demandé : '+new Date(exported).toLocaleString('fr-FR'):'Aucun export demandé. Garde une copie hors de ce navigateur.';list.replaceChildren();
 for(const row of R.backups()){const b=document.createElement('button');b.className='secondary';b.textContent=new Date(row.date).toLocaleString('fr-FR')+' · '+row.reason;b.onclick=async()=>{
  try{R.validate(row.bundle);if(await preview('Restaurer cette version ?',label(row.bundle)+'\nLes données actuelles seront remplacées.','Restaurer')){window.state=R.restore(row.bundle);initControls();renderAll();refresh();notice('Version restaurée. Reconnecte Google si nécessaire.')}}catch(e){notice(e.message)}
 };list.appendChild(b)}
...
function install(){
 ...
 window.exportFull=function(){try{const bundle=R.capture();download('Chef-Secteur-sauvegarde-'+new Date().toISOString().replace(/[:.]/g,'-')+'.json',JSON.stringify(bundle,null,2),'application/json');localStorage.setItem(R.keys.EXPORT,new Date().toISOString());refresh();notice('Téléchargement demandé. Vérifie que le fichier est bien enregistré dans Fichiers.')}catch(e){notice(e.message)}};
 window.importJSONFile=async function(file){if(!file||busy)return;busy=true;try{if(file.size>20*1024*1024)throw Error('Fichier trop volumineux (20 Mo maximum).');const bundle=R.decode(await file.text(),state);if(await preview('Restaurer cette sauvegarde ?',label(bundle)+'\nUne copie des données actuelles sera conservée avant remplacement.','Restaurer')){window.state=R.restore(bundle);initControls();renderAll();refresh();notice('Restauration terminée : '+label(bundle))}}catch(e){notice('Import refusé : '+e.message);showError('Import refusé : '+e.message)}finally{busy=false;const input=document.getElementById('restoreBackupFile');if(input)input.value=''}};
