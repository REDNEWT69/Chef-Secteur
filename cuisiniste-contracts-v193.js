...
function emptyStore(){return{schema:1,trackingImports:[],tariffImports:[],mapping:{}}}
function readStore(storage){try{const raw=storage&&storage.getItem(STORE_KEY);if(!raw)return emptyStore();const p=JSON.parse(raw)||{};return{schema:1,trackingImports:Array.isArray(p.trackingImports)?p.trackingImports:[],tariffImports:Array.isArray(p.tariffImports)?p.tariffImports:[],mapping:p.mapping&&typeof p.mapping==='object'?p.mapping:{}}}catch(e){return emptyStore()}}
...
function renderStoreCard(){if(!root.document)return false;const sh=root.document.getElementById('storeQuickSheet'),start=root.document.getElementById('srQuickStart');if(!sh||!start||!start.dataset)return false;const storeId=text(start.dataset.srStart);let old=root.document.getElementById(STORE_CARD_ID);if(!storeId){if(old)old.remove();return false}const x=briefingForStore(storeId);if(!x){if(old)old.remove();return false}const anchor=root.document.getElementById('sqPerformance')||root.document.getElementById('sqVisitCredit')||root.document.getElementById('sqAddress');if(!anchor)return false;if(!old){ensureStyle();old=el('section');old.id=STORE_CARD_ID;old.setAttribute('aria-label','Contrat expo du magasin');anchor.insertAdjacentElement('afterend',old)}old.replaceChildren();const c=x.activeContract||x.lastContract,badge=el('span',x.activeContract?(c.status||'Contrat expo'):'Historique','srCuisineBadge'+(/alerte/i.test(c&&c.status||'')?' srCuisineAlert':''));old.append(badge,el('b','Contrat Expo · '+siteLabel(x)));if(c){const line=t=>old.append(el('span',t,'srCuisineMeta'));line(x.activeContract?'Contrat actif':'Aucun contrat actif trouvé · dernier contrat '+(c.status||''));line('Période '+(c.startDate||'—')+' → '+(c.endDate||'—'));line('Objectif '+fmtEuro(c.objective)+' · réalisé '+fmtEuro(c.realized)+' · '+fmtPct(c.progress));if(c.monthsRemaining!=null&&typeof c.monthsRemaining==='number')line('Temps restant : '+c.monthsRemaining+' mois');if(c.products&&c.products.length)line('Produits expo : '+c.products.map(p=>p.info?(p.ref+' · '+[p.info.family,p.info.segment].filter(Boolean).join(' / ')):p.ref).join(' · '));line('Signal : '+x.urgency.reason)}old.append(btn('Ouvrir le suivi contrats expo',open));return true}
