(function(){
  ...
  function renderTabs(){const box=document.getElementById('dayTabs');if(!box)return false;const r=range(),frag=document.createDocumentFragment();box.innerHTML='';box.classList.add('periodDayTabs');
    ...
    if(!activeDate||!box.querySelector('[data-date="'+activeDate+'"]')){activeDate=iso(r.start);const first=box.querySelector('[data-date="'+activeDate+'"]')||box.firstElementChild;if(first){first.classList.add('active');activeDate=first.dataset.date||activeDate}}
  ...
  window.addEventListener('chef-range-generated',function(e){const d=e&&e.detail||{};if(d.start)activeDate=d.start;setTimeout(renderTabs,20)});
