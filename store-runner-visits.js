...
function name(storeId){const s=window.state.stores.find(x=>String(x.id)===String(storeId))||domain().storeSnapshots[storeId];return s?s.enseigne+' · '+s.ville:'Magasin archivé'}
...
function activeFamily(v){return M.FAMILIES.includes(v&&v.activeFamily)?v.activeFamily:'brun'}
function shownFamily(v){return v.status==='completed'&&M.FAMILIES.includes(previewFamily)?previewFamily:activeFamily(v)}
function familySwitch(host,v){
 const box=element('div',undefined,'sr-familySwitch'),active=shownFamily(v);
 for(const family of M.FAMILIES){
  ...
  const b=button(family.toUpperCase(),change,'sr-familyBtn');
 ...
 const state=element('p',label+active.toUpperCase()+hint,'sr-familyActive');state.setAttribute('aria-live','polite');box.append(state);host.append(box)
}
