...
function boulangerCount(route){return route.filter(s=>/\bboulanger\b/.test(norm(currentStore(s).enseigne))).length}
...
function fixedError(day,route,max){const actual=(route||[]).reduce((n,s)=>n+actualCredit(s),0),cap=routeCapacity(route),rows=(route||[]).map(s=>storeName(s)+' ('+actualVisitCreditSafe(s)+')');let m=day+' contient déjà '+actual+' crédit'+(actual>1?'s':'')+' fixe'+(actual>1?'s':'')+(rows.length?' : '+rows.join(' + '):'')+'. Ton maximum est réglé sur '+max+'. ';if(actual>max)m+=(actual<=8?'Passe-le à '+actual+' dans Réglages ou libère une visite. ':'Augmente le maximum dans Réglages si c’est volontaire, ou libère une visite. ');else if(cap>max||boulangerCount(route)>1)m+='La règle Boulanger n’autorise qu’un seul magasin à 1 crédit à ses côtés. Libère ou déplace le magasin incompatible. ';return m+'Rien n’a été changé.'}
...
function build(){
  ...
  const mon=weekMonday(),weekKey=iso(mon),today=iso(new Date()),days=workDays(),max=Math.max(1,Math.min(8,Number(state.settings&&state.settings.maxVisitsPerDay)||4)),archive=load(ARCHIVE_KEY,{}),source={},weeks={},entries={},movable=[];
  ...
  /* Les seules journées déjà impossibles à cause de contraintes fixes restent une erreur
     explicite, comme avant. Une visite non fixe n'est jamais sacrifiée pour masquer ce cas. */
  for(const [key,plan] of Object.entries(weeks)){const wm=parse(key);for(const day of DAYS){const date=dayDate(wm,day),route=plan[day]||[];if(date>=today&&(routeCapacity(route)>max||boulangerCount(route)>1))return{ok:false,error:fixedError(day,route,max)}}}
...
async function recalc(){
  ...
  try{
    ...
    if(result.unchanged){status('Planning déjà stable ✓ Aucun déplacement nécessaire.','ok');return result}
    ...
    status('Planning recalculé ✓ '+result.moved+' visite'+(result.moved>1?'s':'')+' déplacée'+(result.moved>1?'s':'')+' · '+result.stableKept+' visite'+(result.stableKept>1?'s':'')+' future'+(result.stableKept>1?'s':'')+' laissée'+(result.stableKept>1?'s':'')+' en place'+spill+'.','ok');
    return result;
