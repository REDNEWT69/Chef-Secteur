...
Cette structure permet notamment au futur domaine Visit/Action d’ajouter son contexte ou ses réponses sans prendre possession des fonctions historiques de l’assistant.
...
Les tests `tests/architecture.test.cjs` et `tests/assistant-architecture.test.cjs` verrouillent notamment les propriétaires de `generateWeek`, `syncGoogleCalendar`, du profil/départ, plusieurs fonctions de rendu et les extensions assistant nettoyées.
...
- `src/chef-secteur.html` reste le noyau historique et contient encore beaucoup de logique globale. Les extractions doivent rester progressives, ciblées et sans migration de framework.
- Le prochain gros chantier fonctionnel est le domaine métier V2 Visit + Action + workflow 6P. Il doit utiliser les propriétaires et contrats ci-dessus plutôt que réintroduire des overrides.

## Ordre de reprise recommandé

1. Conserver le checkpoint architecture vert avant chaque lot fonctionnel.
2. Intégrer Visit + Action avec sauvegarde/reprise, 6P, historique et compatibilité des données existantes.
